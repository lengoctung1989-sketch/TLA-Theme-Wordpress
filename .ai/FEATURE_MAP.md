...
| **P6.2** composer | Khai báo autoload PSR-4 và toàn bộ công cụ kiểm tra PHP (phpcs, phpstan). Là điểm để `composer require` khi nhân bản base theme sang site mới. | `composer.json` | PSR-4 `TL\Theme\` → `src/`. require-dev: php_codesniffer, wpcs, phpcompatibility-wp, phpstan ^2, phpstan-wordpress ^2. scripts `lint`/`lint:fix`/`analyze` |
