...
| **CP3.2** single-product | Skin trang chi tiết sản phẩm: pagehero, `.cp-single` 2 cột, nhãn danh mục, nút hotline cạnh add-to-cart, dải CTA, tabs + related reskin CSS. | `inc/woocommerce.php` | Guard `cp_is_single_product()`. `cp_single_open/close` (pagehero + `.cp-section`; close in ra dải CTA). `cp_single_cat_label` @ `woocommerce_single_product_summary` prio 4. `cp_single_hotline_btn` @ `woocommerce_after_add_to_cart_button`. Hook loop item (CP3.1) chạy chung để related thành `.cp-card` |
| | | `assets/caophat.css` | `.woocommerce div.product` grid 2 cột (gallery \| summary), tabs + related full-width; reskin `.summary`, `p.price`, `form.cart`, `.product_meta`, `ul.tabs`, `.woocommerce-Tabs-panel`, `table.shop_attributes`, `.related ul.products` → `.cp-card` |
