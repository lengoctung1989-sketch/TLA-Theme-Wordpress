...
| **P2.3** performance | Cắt phần thừa ở tầng trình bày: bỏ hẳn wp-emoji (**cả wp-admin**, không chỉ front-end — `disableEmoji()` phải chạy ở cả `init` lẫn `admin_init`, xem chú thích trong `src/Features/Performance.php`; KHÔNG đặt `emoji_svg_url` = false vì twemoji sẽ rơi về CDN jsdelivr 404 ⇒ emoji trong admin thành ảnh vỡ, sự cố 2026-09-18), khai báo preload cho stylesheet chính. Không đụng page cache / HTTP header (việc của hosting). Preload font: thêm khi site thực sự dùng woff2. | `src/Features/Performance.php` | `disableEmoji()` gỡ **6 action** (`wp_head` prio 7 · `embed_head` · `admin_print_scripts` · `wp_print_styles` · `admin_print_styles` · `admin_enqueue_scripts`) + **3 filter** (`the_content_feed` · `comment_text_rss` · `wp_mail`) + plugin `wpemoji` của TinyMCE. `preloadResources()` hook `wp_preload_resources`: preload style `tl-theme-0` |
